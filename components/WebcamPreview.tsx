...
const WebcamPreview: React.FC<WebcamPreviewProps> = ({ videoRef, resultsRef, isCameraReady, colors }) => {
    ...
    }, [isCameraReady, videoRef, resultsRef, colors]);

    if (!isCameraReady) return null;

    return (
        <div 
          className="fixed bottom-4 right-4 w-64 h-48 bg-black/60 border-2 rounded-xl overflow-hidden backdrop-blur-md z-50 shadow-[0_0_20px_rgba(0,0,0,0.5)] pointer-events-none transition-opacity duration-500"
          style={{ borderColor: `${colors.world.text}4D` }} // 30% opacity
        >
             {/* Header/Label */}
            <div className="absolute top-0 left-0 right-0 bg-black/40 text-[10px] px-2 py-1 font-mono uppercase tracking-widest" style={{ color: colors.world.text }}>
                Tracking Feed
            </div>

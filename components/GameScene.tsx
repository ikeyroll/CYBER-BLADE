...
import { GameStatus, NoteData, HandPositions, ColorPalette, CutDirection } from '../types';
...
const GameScene: React.FC<GameSceneProps> = ({ 
  ...
  useEffect(() => {
      // Deep copy chart to ensure we have fresh note objects we can mutate (hit/missed flags)
      // JSON parse/stringify is simple and effective for this plain data structure
      if (chart.length > 0) {
  ...
  useFrame((state, delta) => {
    ...
    for (let i = activeNotesRef.current.length - 1; i >= 0; i--) {
        ...
        // Calculate current Z position based on time and variable speed
  ...
  return (
    <>
      ...
      <color attach="background" args={[colors.world.sky]} />
      <fog attach="fog" args={[colors.world.sky, 10, 50]} />
      
      {/* Pulsing Lights */}
      <ambientLight ref={ambientLightRef} intensity={0.2} color={colors.world.gridAccent} />
      <spotLight ref={spotLightRef} position={[0, 10, 5]} angle={0.5} penumbra={1} intensity={1} castShadow color="white" />
      
      <Environment preset="night" />

      {/* Floor / Track visuals */}
      <Grid position={[0, 0, 0]} args={[6, 100]} cellThickness={0.1} cellColor={colors.world.grid} sectionSize={5} sectionThickness={1.5} sectionColor={colors.world.gridAccent} fadeDistance={60} infiniteGrid />
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.01, 0]}>
          <planeGeometry args={[4, 100]} />
          <meshStandardMaterial color="#000" roughness={0.8} metalness={0.5} />
      </mesh>
      
      <Stars radius={50} depth={50} count={2000} factor={4} saturation={0} fade speed={1} />

      <Saber type="left" positionRef={leftHandPosRef} velocityRef={leftHandVelRef} colors={colors} />
      <Saber type="right" positionRef={rightHandPosRef} velocityRef={rightHandVelRef} colors={colors} />
